...
func TestQueryTimeoutWithTables(t *testing.T) {
	...
	require.Error(t, err)
	assert.Regexp(t, "context deadline exceeded|maximum statement execution time exceeded", err.Error())
	assert.Regexp(t, "\\(errno 1317\\) \\(sqlstate 70100\\)|\\(errno 3024\\) \\(sqlstate HY000\\)", err.Error())
